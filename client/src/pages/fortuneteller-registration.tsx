    ...
    return (
      <label className="block text-sm">
        <div className="flex items-center gap-1 flex-wrap">
          ...
          {maxLen && <span className="text-[10px] text-white/40 ml-auto">{fields[field].length}/{maxLen}</span>}
        </div>
    ...
    <div className="min-h-screen bg-[radial-gradient(1200px_600px_at_50%_-10%,#3a1777_0%,#13254a_45%,#0c1a33_100%)] text-white flex items-start justify-center p-6 pt-10">
      <div className="w-full max-w-md bg-white/5 border border-white/10 rounded-2xl p-6 shadow-xl space-y-5">
        <div className="text-center">
          <div className="text-xl font-bold" data-testid="text-page-title">占い師 新規登録</div>
          <div className="text-xs text-white/50 mt-1">占い師としてのプロフィール情報を入力してください</div>
        </div>
        {serverError && (
        ...
        <div className="space-y-3">
          ...
          <label className="block text-sm">
            <div className="flex items-center gap-1 flex-wrap">
              <span className="text-white/80">自己紹介</span>
              <span className="text-[10px] bg-red-500 text-white px-1.5 py-0.5 rounded-md">必須</span>
            </div>
            ...
            {errors.intro && <p className="text-red-400 text-xs mt-0.5">{errors.intro}</p>}
          </label>
        </div>
